const firebaseConfig = {
    ...
    messagingSenderId: "197140262797",
    appId: "1:197140262797:web:4a03ac108230694ea9d7c4"
};
...
function setupDeviceUI() {
    ...
    if (!isTouchDevice) {
        ...
        let footerControls = document.getElementById('pcControlsFooter');
        if (!footerControls) {
            footerControls = document.createElement('div');
            footerControls.id = 'pcControlsFooter';
            footerControls.style.textAlign = 'center';
            footerControls.style.marginTop = '15px';
            footerControls.style.color = '#d8dee9';
            footerControls.style.fontSize = '14px';
            footerControls.innerHTML = '<strong>Controles PC:</strong> WASD / Flechas (Mover) | Espacio / P / Esc (Pausar) | Enter (Salir en Pausa)';
            canvas.parentNode.insertBefore(footerControls, canvas.nextSibling);
        }
    }
}
...
document.addEventListener('keydown', (event) => {
    ...
    } else if (event.key === 'p' || event.key === 'P' || event.key === ' ' || (event.key === 'Escape' && !isInfoVisible)) {
        event.preventDefault();
        togglePause();
    }
});
